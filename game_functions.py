...
def check_keyup_events(event, ship):
    ...
    # Move the ship to the right
        ship.moving_right = False # ship 파일의 init(초기)함수의 moving_right flag(누를동안 움직이는 속성)가 Flase이므로 정지함.
    elif event.key == pygame.K_LEFT:
...
def update_screen(invasion_settings, screen, ship, younggis, bullets):
...
def update_bullets(invasion_settings, screen, ship, younggis, bullets):
    ...
    check_bullet_younggi_collisions(invasion_settings, screen, ship, younggis, bullets)

def check_bullet_younggi_collisions(invasion_settings, screen, ship, younggis, bullets):
    ...
    if len(younggis) == 0:  # younggis에 변수로 지정한 pygame.sprite의 group() 메서드의 길이가 0이라는 뜻은 the group younggis가
        # 빈값(empty)이라는 뜻으로 기존의 미사일을 없애고, 새 전함(fleet)를 생성함.
...
def check_fleet_edges(invasion_settings, younggis):
    ...
    for younggi in younggis.sprites():
        if younggi.check_edges():
            ...
            break
        # younggis(비행체)가 화면의 edge(구석)에 닿는지를 체크하는 함수로 닿으면 change_fleet_direction 함수를 호출하고
        # loop문을 빠져나옴(break).
...
def change_fleet_direction(invasion_settings, younggis):
    ...
    # younggi image의 사각형이 invasion_settings.fleet_drop_speed값 '3'(setting값)만큼 증가하면서 loop문 이후 fleet_direction
    # 현재값에 -1을 곱해서 값의 부호를 변환, 즉 방향이 전환됨(좌측에서 우측 혹은 우측에서 좌측)

def ship_hit(invasion_settings, stats, screen, ship, younggis, bullets):
...
def check_younggis_bottom(invasion_settings, stats, screen, ship, younggis, bullets):
    """Check if any younggis have reached the bottom of the screen."""
    screen_rect = screen.get_rect()
    for younggi in younggis.sprites():
        if younggi.rect.bottom >= screen_rect.bottom:
            # Treat this the same as if the ship got hit.
            ship_hit(invasion_settings, stats, screen, ship, younggis, bullets)
            break

def update_younggis(invasion_settings, stats, screen, ship, younggis, bullets):
    """
    ...
    younggis.update()
    # update_younggis 함수를 통해 구석에 닿는 younggis 전함(fleet)이 check_fleet_edges()를 호출하는 시키는 여부를 결정함. 이 함수는
    # invasion_settings 형식 파라미터가 필요하고, 그래서 invasion_settings에 필요한 인자(Settings 클래스의 세팅값)를 포함시켜서
    # update_younggis() 함수를 호출시킬 수 있도록 함.
    ...
    check_younggis_bottom(invasion_settings, stats, screen, ship, younggis, bullets)